...
let extract_policy cacert =
  match X509.(Extension.(find (Unsupported Vmm_asn.oid) (Certificate.extensions cacert))) with
  | None -> Ok None
...
let () =
  let doc = "Albatross CA operations" in
  let man = [
    `S "DESCRIPTION" ;
    `P "$(tname) does CA operations (creation, sign, requesting CSR, etc.)" ]
  in
